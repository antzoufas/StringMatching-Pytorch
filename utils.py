...
def train(model, data, data_val):
    ...
    for epoch in range(n_epochs):
        ...
        for i, batch in enumerate(data):
            ...
            loss.backward()
            optimizer.step()
        print("Epoch {}/{}, Training Loss: {}".format(epoch+1, n_epochs, total_loss))
        if total_loss < best_loss:
            ...
            if num_epochs_worse == patience:
                model.load_state_dict(torch.load('current_best.pt'), strict=False)
...
def display_results(results1, results2):
    if results2 == None:
        results2 = results1
    print("Validation Accuracy = {}".format((results1[0]+results2[0]) / 2))
    print("Validation Precision = {}".format((results1[1]+results2[1]) / 2))
    print("Validation Recall = {}".format((results1[2]+results2[2]) / 2))
    print("Validation F1 = {}".format((results1[3]+results2[3]) / 2))
    print()
